        ...
        return (
            <div className="addQuiz">
                ...
                <form className="formAdd" onSubmit={handleSubmit}>
                    <input style={{ width: '80%', maxHeight: '30px', minHeight: '30px' }} type="text" onChange={e => this.setState({ name: e.target.value })} placeholder="Add new quiz..." />
                    ...
                    <thead style={{ height: '45px' }}>
                        {this.state.quiz.map(quiz => (
                            <tr key={uuidv4()}>
                                ...
                                <td style={{ width: '70px' }}> <button id='del' type="submit" onClick={() => handleDelete(quiz.id)}>Delete</button></td>
                                <td style={{ width: '160px' }}><Link to={`/add/question/${quiz.id}`} ><button id='addNew' >Add Question</button></Link></td>
                            </tr>
                        ))}
                    </thead>
                </table>
